...
# --- STAP 2: DE ESM LOOP MET EXTRA FOUTCONTROLE ---
results_list <- list()
counter <- 1
    ...
    if(nrow(est_df) >= 150 && all(is.finite(est_df$R_bank))) {
      ...
      if(!inherits(fit, "try-error")) {
        ...
        results_list[[counter]] <- data.frame(
          ...
          CAR          = sum(ev_df$AR, na.rm = TRUE),
          Tightness    = relevant_speeches$Tightness[i],
          Regulation   = relevant_speeches$Reg_Score[i],
          Supervision  = relevant_speeches$Sup_Score[i],
